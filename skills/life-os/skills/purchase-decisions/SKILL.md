...
description: Support thoughtful purchase decisions through requirements, comparisons, total cost, risk, timing, and post-purchase tracking.
version: 0.1.0
...
Make purchases less dumb. This skill turns vague buying impulses into requirements, budget, alternatives, total cost, timing, warranty risk, second-hand checks, and a clear recommendation.

## Trigger

Use when the user asks to:

- decide whether to buy something
- compare products, services, vendors, or second-hand listings
- define requirements and non-negotiables
- track warranty, returns, delivery, or post-purchase follow-up
- defer a purchase until price, stock, or need changes
...
8. Record useful Life OS coordination state under `$LIFEOS_DATA_DIR/purchase-decisions/data.json` when the user/runtime policy allows it.

## Source decision

Store the source choice in `$LIFEOS_DATA_DIR/purchase-decisions/data.json` when useful:
...
```text
$LIFEOS_DATA_DIR/purchase-decisions/data.json
```
