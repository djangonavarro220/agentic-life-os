...
description: Record important decisions, options, tradeoffs, assumptions, review dates, and outcomes.
version: 0.1.0
...
Record important decisions so future agents and humans can understand the context, options, tradeoffs, assumptions, review date, and outcome. It is a memory prosthetic for judgment.
...
- set a future review date for a decision
- review whether a past decision worked
- capture assumptions before committing to a plan
...
8. Record useful Life OS coordination state under `$LIFEOS_DATA_DIR/decision-journal/data.json` when the user/runtime policy allows it.

## Source decision

Store the source choice in `$LIFEOS_DATA_DIR/decision-journal/data.json` when useful:
...
```text
$LIFEOS_DATA_DIR/decision-journal/data.json
```
