# Hermes adapter for work-portfolio
...
Ask before changing Hermes cron jobs, gateway delivery, channel prompts, profile config, memory providers, vault entries, mail/calendar state, or external services. Store Life OS pointers in `$LIFEOS_DATA_DIR/config.json` or `$LIFEOS_DATA_DIR/work-portfolio/data.json`, not in public repo files.
