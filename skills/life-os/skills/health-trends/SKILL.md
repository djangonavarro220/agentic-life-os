---
name: health-trends
description: Track health-related trends, symptoms, habits, medication notes, and care questions without giving medical diagnosis.
version: 0.1.0
...
Track health-related signals as a pattern and preparation layer. This skill helps notice trends, prepare care questions, and remember routines. It does not diagnose, prescribe, or replace a qualified clinician.

## Trigger

Use when the user asks to:

- track symptoms, sleep, exercise, nutrition, weight, medication notes, or health routines
- review health trends over time
- prepare questions for a clinician or appointment
- notice whether a habit or symptom changed
- decide whether a health item needs a reminder, task, or calendar entry
...
8. Record useful Life OS coordination state under `$LIFEOS_DATA_DIR/health-trends/data.json` when the user/runtime policy allows it.

## Source decision

Store the source choice in `$LIFEOS_DATA_DIR/health-trends/data.json` when useful:
...
```text
$LIFEOS_DATA_DIR/health-trends/data.json
```
